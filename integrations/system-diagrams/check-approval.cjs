...
assert.match(approval.postconditions.join(' '),/sets Approved and permits laboratory processing without another Dean decision/);
...
assert.match(faculty.response,/approval of either eligible schedule variant sets Approved; no subsequent Dean review is required/);
const dean=data.events.find(e=>e.diagramId==='approve'&&e.source==='Dean');
assert.match(dean.trigger,/directly routed because the assigned Faculty is unavailable/);
...
assert.match(data.useCases.find(u=>u.diagramId==='viewstatus').briefDescription,/either Faculty or Dean according to the applicable routing rule/);
