...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;try{
 ...
 assert.equal(await page.locator('.erd-sheet svg > rect,.erd-sheet svg > text').count(),0,'No outer frame, title, description or legend inside the ERD artwork');
 ...
 for(const g of Object.values(geometry)){
  assert.equal(g.row,25,'Additional clearance between FK row endpoints');
 ...
 if(process.argv.includes('--render')){await page.emulateMedia({media:'print'});await page.pdf({path:path.join(root,'assets/erd/erd.pdf'),preferCSSPageSize:true,printBackground:true});const pdf=fs.readFileSync(path.join(root,'assets/erd/erd.pdf')).toString('latin1');assert.equal((pdf.match(/\/Type \/Page\b/g)||[]).length,1,'One complete A4 page');const box=pdf.match(/\/MediaBox\s*\[\s*0\s+0\s+([\d.]+)\s+([\d.]+)/);assert(box&&Math.abs(Number(box[1])-595.28)<1&&Math.abs(Number(box[2])-841.89)<1,'A4 portrait PDF dimensions');for(const s of model.sheets){const svg=page.locator('#erd-'+s.id+' svg');fs.writeFileSync(path.join(root,'assets/erd/erd-'+s.id+'.svg'),await svg.evaluate(n=>new XMLSerializer().serializeToString(n)));await svg.screenshot({path:path.join(root,'assets/erd/erd-'+s.id+'.png')});}}
 console.log('PASS:',model.tables.length,'entities;',total,'PK–FK relationships; all ten DFD stores;',model.sheets.length,'complete A4 portrait sheet. Logical draft, not approval.');
