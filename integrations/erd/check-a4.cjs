...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;try{
 ...
 for(const g of Object.values(geometry))for(let i=0;i<g.paths.length;i++)for(let j=i+1;j<g.paths.length;j++){
  ...
  for(const [a,b] of segments(p))for(const [c,d] of segments(q)){
   ...
   if(shared>40&&gap<6){closeRuns++;if(gap<.1)mergedRuns++;console.log('Close routes',p.id,q.id,{a,b,c,d,gap,shared});}
  }
 }
 ...
 assert.equal(closeRuns,0,'Long parallel runs maintain at least 6 SVG units of separation');
 for(const sheet of model.sheets){const g=geometry[sheet.id],nodes=new Map(g.nodes.map(n=>[n.name,n]));
  assert.equal(new Set(g.nodes.map(n=>n.x)).size,5,'Flexible A4 layout uses five routing columns');
  assert(Math.max(...g.nodes.map(n=>n.y+n.h))-Math.min(...g.nodes.map(n=>n.y))>1900,'Entities use the full portrait height');
  for(let i=0;i<g.nodes.length;i++)for(let j=i+1;j<g.nodes.length;j++){const a=g.nodes[i],b=g.nodes[j];assert(a.x+a.w<=b.x||b.x+b.w<=a.x||a.y+a.h<=b.y||b.y+b.h<=a.y,a.name+' does not overlap '+b.name);}
  ...
  for(const p of g.paths){const a=nodes.get(p.parent),b=nodes.get(p.table),start=p.points[0],end=p.points.at(-1),parentMarker=g.markerBoxes.find(m=>m.id===p.id&&m.end==='parent');assert.equal(start[1],parentMarker.y+18,'Parent perimeter port aligns with its cardinality');assert(start[1]>=a.y&&start[1]<=a.y+a.h,'Parent port stays on its table');assert.equal(end[1],b.y+g.header+b.fields.indexOf(p.field)*g.row+g.row/2,'Child FK row attachment');assert([a.x,a.x+a.w].includes(start[0]));assert([b.x,b.x+b.w].includes(end[0]));
   ...
   for(let i=0;i<p.points.length-1;i++){const u=p.points[i],v=p.points[i+1];for(const m of g.markerBoxes){if(m.id===p.id)continue;const cross=u[0]===v[0]?u[0]>m.x&&u[0]<m.x+m.w&&Math.max(u[1],v[1])>m.y&&Math.min(u[1],v[1])<m.y+m.h:u[1]>m.y&&u[1]<m.y+m.h&&Math.max(u[0],v[0])>m.x&&Math.min(u[0],v[0])<m.x+m.w;assert(!cross,p.id+' crosses cardinality '+m.id+' '+m.end);}}
  }
 }
 ...
 assert(await page.locator('[data-cardinality] circle').evaluateAll(nodes=>nodes.every(n=>Number(n.getAttribute('r'))===4)),'Readable optionality circles');
 assert(await page.locator('[data-cardinality] line').evaluateAll(nodes=>nodes.every(n=>Math.abs(Number(n.getAttribute('y2'))-Number(n.getAttribute('y1')))<=10)),'Cardinality bars/prongs are compact');
 ...
 assert.equal(await focusMark.locator('.cardinality-backplate').evaluate(n=>getComputedStyle(n).fill),'rgb(220, 236, 255)','Endpoint has a visible highlight background');
 ...
 console.log('PASS:',model.tables.length,'entities;',total,'PK–FK relationships; all eleven DFD stores;',model.sheets.length,'complete interactive A4 portrait sheet. Logical draft, not approval.');
