...
Use this only on a personal, trusted device: browser local storage is not encrypted and scripts on the same website can read it. Storage is separate for each browser/profile and website origin (including localhost port); private browsing or clearing site data can remove it. If storage is blocked, manual sync still works. Google authorization may occasionally need renewal.

## A group member cannot update

Downloading GitHub files and knowing the sync key does not grant Google access. Under the recommended **Only myself** deployment, only the deploying Google account can use that web app—even if another member can edit the target Google Doc. See [Google's web app permissions](https://developers.google.com/apps-script/guides/web#permissions).

Each member should:

1. Obtain **Editor** access to the target document using their own Google account.
2. Follow the one-time setup above in their **own Apps Script project**: run `setupSync`, authorize document/Drive access, and deploy privately as **Me / Only myself**.
3. Use their own `/exec` URL and the key generated by that same project, while signed into that Google account. The owner's key will not match a member's separate project.
4. Open the `/exec` URL directly first. If Google denies access, check the signed-in account and deployment access before retrying. Use Live Server for Docs.html and allow its result popup. `/dev` is a testing URL, not the deployment URL.

Do not make the owner's web app public to bypass access errors. School account policies may require administrator help. If access works but the update fails, capture the exact Google result/error message without including the key.

Coordinate **one sync at a time across members**: script locks are per Apps Script project, not shared across separate deployments. Pull the latest local content first and review the selected sections; sync replaces content rather than merging other members' edits.

If a key has been shared in chat or exposed, rotate it in that project's **Project Settings > Script properties** by deleting `LAB_SYNC_KEY`, then run `setupSync` to generate a replacement. Running setupSync without deleting the property returns the old key. Save the replacement locally; old saved keys will no longer work.

## Text, tables, and diagrams

- Each checked section's **contents** are replaced using its existing heading as an anchor. Missing selected Activity/Swimlane sections are inserted after the existing ERD, in canonical order. Unchecked text, cover authors and header/footer content are preserved, but **all first-tab body/header/footer text is standardized to 11 pt**. Other tabs are untouched.
...
- The first document tab must contain the standard section headings through ERD. Only the new Activity and Swimlane headings may be absent and created automatically. Duplicate or out-of-order diagram headings are rejected. The title anchor accepts the old title or the current local title. Arbitrary Google title changes require updating the anchor first.

## Formatting and review

Body paragraphs are justified. Before replacement, the sync samples an existing normal body paragraph and reuses its font family, text color, indents, line spacing and paragraph spacing; font size is always overridden to 11 pt. If no suitable paragraph exists, the fallback is Times New Roman 11 pt with 8 pt space after. After selected updates, the first tab's complete body, header and footer receive 11 pt, including unselected text. It never reads or changes the original document.

Inline bold and italic formatting from the rendered local page is retained in paragraphs, headings, and table cells. The cover title remains bold and retains its font family and alignment, but becomes 11 pt like headings, captions and table text. Existing main headings keep their text and heading level. Captions and figures remain centered; lists and new subheadings are left-aligned. Tables do not inherit body justification. Text embedded in PNG/SVG diagrams cannot be resized by a Google Docs font setting.

### Activity and Swimlane sync (new deployment required)

Replace the deployed script with the current `Code.gs`, then choose **Deploy → Manage deployments → Edit → New version → Deploy**. This release identifies itself as `2026-09-16-activity-swimlane-font11`. The existing `/exec` URL and private key can stay the same when updating that deployment. The client now sends payload version 3; an older server rejects it before making changes.

- **3.1.9 Activity Diagrams:** five PNGs, Figures 10–14, one for each DFD parent process.
- **3.1.10 Swimlane Diagram:** one whole-system PNG, Figure 15.
- Both are independently selectable in Update Google Docs. New headings are created only when their section is selected; subsequent syncs replace their contents without duplicating headings.
- New section headings and subsequent activity subheadings include a page break in their own paragraph. This preserves the break when later replacing adjacent section contents. [DocumentApp supports paragraph page breaks](https://developers.google.com/apps-script/reference/document/paragraph#appendPageBreak()). Review final pagination in Google Docs after syncing.
- The local paper has six new numbered pages. Diagram exports are 2400 pixels wide, include their SVG styling, and remain separate from their figure captions.
- Refresh the Google Docs table of contents after syncing. This workflow does not claim to reproduce the browser's page numbering exactly.
...
Removed three Laboratory Dashboard presentation arrows, two staff Daily Task Entry arrows, and—under the latest user-approved role correction—two staff Usage Entry arrows (81 → 74). Dashboard screens remain a feature; they are intentionally omitted from DFD boundary presentation. Only Head Laboratory sends Schedule Update, Usage Entry and Daily Task Entry; staff do not manage laboratory logs, schedules or daily tasks. Head Laboratory is last in the Level 1 entity column. The matching Level 0 has 46 flows. After the end-term report clarification, Level 1 has 72 flows: the D7 daily-task and D10 disposal reads into reporting are removed. Level 2 Process 5 exposes the same 15 retained parent boundary exchanges, including eight store exchanges, and clearly labels Head-only daily tasks. Its source remains editable and its publication frame remains 1200 × 950. Clearance and daily-task management remain intact; only their inclusion in reporting is excluded.

Run `node integrations/google-docs/check-dfd-scope.cjs --render` for role scope, parent balancing and Level 0 geometry, and to regenerate the portrait Level 0 PNG. Run `node integrations/google-docs/check-level2.cjs --render` for all five Level 2 models, geometry, editor interactions and publication PNGs. P5's 15 parent flows are realized by 17 child exchanges because schedule and usage write at separate stages and clearance now reads borrower evidence from D5. Select Context Diagram and Data Flow Diagrams to sync these refreshed images. Review first; these checks do not update a live Google Doc.
