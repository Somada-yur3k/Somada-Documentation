...
window.SystemPortraitSwimlane=function(id){
 ...
 const marker=el('marker',{id:id+'-arrow',viewBox:'0 0 10 10',refX:9,refY:5,markerWidth:10.5,markerHeight:10.5,orient:'auto'},el('defs',{},svg));el('path',{d:'M1 1 L9 5 L1 9 Z',fill:'#000'},marker);
 ...
 function node(key,lane,y,title,kind='action',process){
  ...
  if(title)text(g,x,logicalY,title,kind==='decision'?26:28,kind==='action');return n;
 }
 ...
 node('start',4,175,'','initial');dec('readiness',4,285,'Setup / admin\nneeded?');act('admin',4,400,'Manage accounts:\nFaculty / Class Rep.','p1');act('admin-tasks',4,490,'Schedule / daily tasks\n(as needed)','p5');node('ready',4,575,'','merge');
 link('start','b','readiness','t');link('readiness','l','admin','l',[[1690,285],[1690,400]],'[Yes]',[1740,355]);link('readiness','r','ready','r',[[2070,285],[2070,575]],'[No]',[2110,520]);
 link('admin','r','admin-tasks','r',[[2090,400],[2090,490]]);link('admin-tasks','r','ready','r',[[2090,490],[2090,575]]);
 dec('question',0,575,'Need Q&A?');act('ask',0,735,'Ask AI; read\nlaboratory guidance','p3');node('request-ready',0,860,'','join');
 ...
 dec('requester',1,575,'Requester?');link('ready','l','requester','r');link('requester','l','question','r',[],'[Class Rep.]',[420,548]);
 act('faculty-request',1,720,'Check slots / ask AI;\nsubmit own request','p2');dec('faculty-scheduled',1,860,'Own on-\nschedule?');link('requester','b','faculty-request','t',[],'[Faculty]',[707,651]);link('faculty-request','b','faculty-scheduled','t');
 ...
 act('select-type',0,892,'Create reservation;\nselect Reservation Type','p2');link('request-ready','b','select-type','t');
 ...
 link('reservation-type','l','type-merge',[110,nodes['type-merge'].y,'raw'],[[45,925],[45,nodes['type-merge'].y,'raw'],[110,nodes['type-merge'].y,'raw']],'[Group]',[90,945]);
 link('reservation-type','r','type-merge',[310,nodes['type-merge'].y,'raw'],[[390,925],[390,nodes['type-merge'].y,'raw'],[310,nodes['type-merge'].y,'raw']],'[Student Only]',[302,945]);
 text(labels,210,972,'{joinSpec = or}',20);
 act('request',0,985,'Submit selected type;\non- / out-of-schedule','p2');link('type-merge','b','request','t');act('review',1,985,'Review request;\nApprove / Reject','p2');link('request','r','review','l');
 dec('faculty-approved',1,1126,'Faculty\napproved?');link('review','b','faculty-approved','t');act('rejected',0,1126,'Read rejected status','p2');node('rejected-end',0,1240,'','final');link('faculty-approved','l','rejected','r',[],'[No]',[416,1096]);link('rejected','b','rejected-end','t');
 ...
 act('dean',5,1460,'Review Pending Dean;\nApprove / Reject','p2');link('dean-route','r','dean','t',[[2480,1250],[2480,1390],[2310,1390]]);link('off-schedule','b','dean-route',[2302,1310],[[630,1385],[2150,1385],[2150,1310]],'[Yes]',[730,1360]);
 dec('dean-approved',5,1605,'Dean\napproved?');link('dean','b','dean-approved','t');act('dean-rejected',0,1605,'Read rejected status','p2');node('dean-rejected-end',0,1730,'','final');link('dean-approved','l','dean-rejected','r',[],'[No]',[2070,1575]);link('dean-rejected','b','dean-rejected-end','t');
 ...
 act('prepare-user',0,2010,'Check final approval;\nprepare for session','p2');act('prepare-faculty',1,2010,'Check final booking;\nprepare for session','p2');
 ...
 link('prepare-physics','b','prepare-join',[770,2152],[[1470,2130],[770,2130]]);
 ...
 text(labels,350,2070,'Rep',22);text(labels,520,2110,'Faculty',22);text(labels,900,2085,'Circuit',22);text(labels,1300,2110,'Physics',22);
 dec('service-lab',2,2320,'Laboratory?');link('prepare-join','b','service-lab','l',[[630,2320]]);act('circuit-service',2,2450,'Issue items;\nreconcile returns','p4');act('physics-service',3,2450,'Issue items;\nreconcile returns','p4');link('service-lab','b','circuit-service','t',[],'[Circuit]',[1130,2387]);link('service-lab','r','physics-service','t',[[1470,2320]],'[Physics]',[1380,2287]);
 ...
 link('physics-service','b','returns',[1130,2557],[[1470,2535],[1130,2535]]);
 text(labels,1050,2670,'{joinSpec = or}',22);
 dec('balance',4,2565,'Unresolved\nbalance?');link('returns','b','balance','l',[[1050,2630],[1720,2630],[1720,2565]]);act('clearance',4,2700,'Manage clearance;\nkeep request Ongoing','p5');node('balance-end',4,2795,'','final');link('balance','b','clearance','t',[],'[Yes]',[1957,2636]);link('clearance','b','balance-end','t');
 act('completed',1,2565,'View Completed status;\nlogs saved automatically','p2');link('balance','t','completed','t',[[1890,2235],[855,2235],[855,2500],[630,2500]],'[No]',[1935,2350]);
