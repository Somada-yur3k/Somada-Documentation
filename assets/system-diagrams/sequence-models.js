...
add({id:'p2',title:'Reservations, Availability and Approvals',source:'p2',actors:['Class Representative','Faculty','Dean'],
 ...
 steps:[
  ...
  opt('Pending Dean review is required',[call('dean','system','Approve / reject eligible request'),self('Validate Faculty approval or direct Faculty-unavailability route'),...db('Save final Approved or Rejected','Final decision saved',['d2'],true),signal('requester','Notify final decision'),reply('system','dean','Final decision confirmed')])
 ],
 note:'Class Representative selects GROUP or STUDENT_ONLY for selected classmates: GROUP retains selected class members; STUDENT_ONLY requires one selected class student. The submitting account is not automatically the borrower. Both schedule variants require the choice. On-schedule Class Representative requests go to Faculty. Out-of-schedule requests go to available Faculty then Dean; they may go directly to Dean only when the assigned Faculty is unavailable. Faculty out-of-schedule requests go to Dean; Faculty scheduled activities need no academic approval.'});
