...
(async function(){
 ...
 try{
  ...
  const pending=data.findings.filter(f=>f.status==='pending'),linked=data.features.filter(f=>f.status==='pass').length,available=data.groups.filter(g=>g.available??(g.status!=='pending')).length,done=data.backlog.filter(b=>/^(done|completed)$/i.test(b.status)).length;
  const metrics=[['Feature trace coverage',percent(linked,data.features.length)+'%',`${linked} / ${data.features.length} main use cases linked`],['Artifact availability',percent(available,data.groups.length)+'%',`${available} / ${data.groups.length} artifacts · includes unapproved ERD draft`],['Open review items',pending.length,'Consultation decisions + ERD validation'],['Recorded implementation',percent(done,data.backlog.length)+'%',`${done} / ${data.backlog.length} backlog items marked complete`]];
  ...
  $('model-grid').innerHTML=data.groups.map((g,i)=>`<a class="model-card ${g.status}" href="${escape(g.href)}"><span class="index">MODEL 0${i+1}</span><h3>${escape(g.name)}</h3><span class="score">${g.percent===null?'Pending':g.percent+'%'}</span><div class="bar" aria-hidden="true"><span style="width:${g.percent||0}%"></span></div><small>${g.percent===null?'Logical draft available; validation pending':`${g.passed} / ${g.total} structural checks`}</small></a>`).join('');
