...
Every printed arrow displays its complete canonical name. The crossing/reference footer note is removed from the publication SVG. Each figure has unique peers, uniform dimensions within each node class, separate ports and paths, and a vertically ordered process column. Logs, schedules and daily tasks remain Head Laboratory only; Q&A remains informational.
