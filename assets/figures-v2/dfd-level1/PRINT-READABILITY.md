...
The approved constraints are one portrait sheet, all 72 separate arrows, no repeated entities or stores, and line crossings allowed only without junctions or shared segments. This is an explicit exception to the DFD arrow skill's default prohibition on crossings; its other integrity rules remain in effect.
...
The user also requested plain labels without boxes or borders, overriding the skill's default pill styling. Only each label's own connector stroke has a transparent gap under the text, leaving the white paper visible; there is no covering rectangle. Clearance extends 6 units beyond each text edge (previously 2). The gap follows label/route edits and is included in PNG and Google Docs exports. The latest vertical-only revision increases separation between coexisting vertical runs to at least 9 units on the actor side (previously 5.5), and 8 units on the store side (previously 5.2). Routes in distant, non-overlapping Y regions can occupy nearby but strictly different X coordinates; no coordinate, segment, bend or port is shared. The latest correction also reserves at least 8 units between horizontal rows, including the Class Representative bend near Faculty Credentials. The editor uses the dfd-level1-full-labels-v5 storage key so older offsets cannot undo this spacing. A rendered-pixel test verifies white padding and visible connector strokes on both sides of all 72 labels.

## Routing and verification

All 72 connector strokes and all actor/store arrowheads now use pure black (`#000000`) in the editor, print, PNG and Google Docs capture. This is a presentation-only change: actor marker IDs, labels, node positions, port assignments and routes are unchanged. Hover tracing remains available without actor-specific arrow colors.
...
Printed arrow names exactly match the canonical model, including On-Schedule Non-Laboratory Request, Account Management Details and Reservation / Approval Record. Account Management Details covers Head Laboratory creation and maintenance of Faculty / Class Representative accounts; Issued Account Credentials is creation-only. Level 1 retains its existing flows; Level 2 now also realizes the existing D1 Account Data read at process 1.3 for account maintenance.
