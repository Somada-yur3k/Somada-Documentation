...
(function(root){
 ...
 function prepare(input,account){
  ...
  if(account.role==='Class Representative'){
   ...
   // The production server must load these assignments, never trust client IDs.
   const assigned=(account.assigned_classes||[]).find(c=>c.group_id===input.group_id&&c.active===true);
   if(!assigned||!assigned.faculty_id)throw Error('Choose an active assigned class with a Faculty reviewer.');
   record.group_id=assigned.group_id;
   record.faculty_id=assigned.faculty_id;
   record.reservation_type=input.reservation_type;
   if(input.reservation_type==='STUDENT_ONLY'){
    if(typeof input.student_id!=='string'||!input.student_id)throw Error('Select one student for this reservation.');
    if(!(assigned.student_ids||[]).includes(input.student_id))throw Error('The selected student must belong to the assigned class.');
    record.student_ids=[input.student_id];
   }else{
    const selected=[...new Set(input.student_ids||[])];
    if(!selected.length)throw Error('Confirm the existing group/member information.');
    if(selected.some(id=>!(assigned.student_ids||[]).includes(id)))throw Error('Members must belong to the assigned class.');
