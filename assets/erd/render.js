(function(){
'use strict';
const model=window.LabErdModel,NS='http://www.w3.org/2000/svg',W=230,HEADER=28,ROW=25,PAGE_W=1400,PAGE_H=1980;
...
function layout(sheet){
 ...
 const nodes=[];
 columns.forEach((names,col)=>{let y=100;for(const name of names){const fields=byName.get(name).fields,h=HEADER+fields.length*ROW;nodes.push({name,x:[55,410,765,1120][col],y,w:W,h,fields,reference:false});y+=h+(name==='RESERVATION'?90:115);}if(y-115>1910)throw Error('ERD exceeds portrait height');});
 return nodes;
}
function route(start,end,nodes,labels,used,parentKey,avoidX=[]){
 const xs=[20,1380,...[297,652,1007].flatMap(x=>Array.from({length:8},(_,i)=>x+i*14)),start[0],end[0]],ys=[74,1920,start[1],end[1]];
...
function diagram(sheet,number){
 ...
 el('style',{},svg).textContent='text{font-family:Arial,Helvetica,sans-serif;fill:#000}path,line{fill:none;stroke:#000;stroke-width:1.5}rect{fill:#fff;stroke:#000;stroke-width:1.4}';
 ...
 for(const n of nodes){
  ...
  n.fields.forEach((f,i)=>{const y=n.y+HEADER+i*ROW;el('line',{x1:n.x,y1:y+ROW,x2:n.x+n.w,y2:y+ROW,style:'stroke-width:.45'},g);
   if(f.key)txt(g,n.x+6,y+16,f.key,13,'start',true);const row=txt(g,n.x+32,y+16,f.name+(f.nullable?' ?':''),17);row.setAttribute('data-field',f.name);
   if(f.ref&&!n.reference){const relation=relations.find(r=>r.table===n.name&&r.field===f.name);txt(g,n.x+n.w-6,y+16,relation.id,12,'end').setAttribute('data-fk-relation',relation.id);}
  });
 }
 ...
 const sides=r=>{const p=map.get(r.parent),c=map.get(r.table);
  // Take the conversation link out of the crowded account/student corridor.
  if(r.table==='CHAT_EXCHANGE'||['DAILY_TASK','USAGE_LOG'].includes(r.table)&&r.field==='recorded_by')return ['right','right'];
 ...
 for(const r of sheetRelations){
  ...
  const outerAccountRoute=r.table==='DAILY_TASK'&&r.field==='recorded_by';
  const a=anchor(parent,r.key,ps),b=anchor(child,r.field,cs),mark=[a[0]+pd*20,a[1]+offset],sa=[a[0]+pd*(outerAccountRoute?90:60),mark[1]],sb=[b[0]+cd*50,b[1]],middle=route(sa,sb,nodes,labelBoxes,used,r.parent,outerAccountRoute?[1055]:[]),points=[a,[a[0]+pd*14,a[1]],[a[0]+pd*14,mark[1]],...middle,b];
