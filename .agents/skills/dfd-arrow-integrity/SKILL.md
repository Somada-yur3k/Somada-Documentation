...
- Declare one `width x height` pair for external entities before laying out a diagram. Every external entity in that diagram, including repeated `*` representations, must use that exact width and height.
...
- Repetition for readability applies only to external entities marked with `*`; data stores are never eligible for repetition.
...
6. Assign a unique source port, destination port, and route lane to each flow.
7. Assign distinct approach tiers to every fan-in or fan-out group; do not place overlapping horizontal intervals on the same Y-coordinate or overlapping vertical intervals on the same X-coordinate.
8. Select a host segment and orientation for every label, then reserve its padded clearance area before drawing paths.
9. Draw only after all flows have independent routes and every store reference resolves to the single canonical symbol.
...
4. Repeat an external entity near another process and mark it with `*` or a duplicate indicator. Add a note that it is the same actor repeated only for readability.
5. Split a crowded diagram into clearly titled panels while preserving the process numbering and flow inventory.

Never solve a conflict by merging arrows, deleting a required flow, hiding a label, or relying on a crossing gap.
Never solve a conflict by resizing only one peer node or by duplicating a data store.
...
7. Inspect every detour. Remove each bend in turn; if the shorter route remains clear, keep the shorter route and reject the detour.
8. Confirm that local flows do not leave the source-destination corridor or cross into an unrelated diagram column.
9. Confirm that every arrow has a visible noun label anchored to its own segment, with the same orientation as that segment and no ambiguous floating placement.
10. Confirm that every label is a text-sized white rounded pill with tight padding, correct horizontal/vertical orientation, and a center point lying on its own connector segment. Verify that it covers only that connector—not a neighboring flow, arrowhead, bend, or node border.
11. Compare the rendered arrows against the flow inventory and the parent DFD.
12. Verify that the set of entity dimensions contains exactly one `width x height` pair, the set of peer-process dimensions contains exactly one pair, and the set of data-store dimensions contains exactly one pair.
13. Verify that all data-store keys and normalized canonical names are unique and that no repeated `*` symbol is a data store.
14. For an interactive SVG, test hover linkage, label dragging, route-handle dragging, persistence, reset, and export mode. Confirm that route edits keep both endpoints fixed on their node boundaries.
15. Do not report completion until every check passes.

When handing off the result, mention any repeated entity or panel split used for clarity and keep the diagram source editable.
